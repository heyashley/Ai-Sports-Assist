...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Swap help images and inject decorative images for Settings and Scores
  // Update Help illustrations
  const swapHelpIllustrations = () => {
    if (bodyHowToUseElem) {
      const imgs = bodyHowToUseElem.querySelectorAll('img');
      if (imgs[0]) imgs[0].src = './img/undraw_pilates_gpdb.svg';
      if (imgs[1]) imgs[1].src = './img/undraw_taking_selfie_re_wlgd.svg';
      // Keep the third image as workout illustration
    }
  };

  const injectDecorImage = (wrapperSel, src, alt) => {
    const card = document.querySelector(`${wrapperSel} > div`);
    if (card && !card.querySelector('[data-decor-img]')) {
      const img = document.createElement('img');
      img.src = src;
      img.alt = alt;
      img.className = 'w-1/3 mt-3 mb-1';
      img.setAttribute('data-decor-img', 'true');
      card.insertBefore(img, card.firstChild);
    }
  };

  swapHelpIllustrations();
  injectDecorImage('#settingsBox', './img/undraw_pilates_gpdb.svg', 'Settings');
  injectDecorImage('#scoresBox', './img/undraw_workout_gcgu.svg', 'Scores');

  // Add user-facing Upload Video buttons and hidden input
  // Top-right bar: insert an Upload button before Settings
  const uploadBtnTop = document.createElement("div");
  uploadBtnTop.className =
    "cursor-pointer hover:bg-gray-200 bg-white text-xl font-bold text-gray-700 py-1 px-3 rounded-lg mr-2";
  uploadBtnTop.textContent = "Upload";
  // unify styling class
  uploadBtnTop.classList.add("control-btn");
  if (settingsBtnElem && settingsBtnElem.parentElement) {
    settingsBtnElem.parentElement.insertBefore(uploadBtnTop, settingsBtnElem);
  }
  // also style existing control chips
  [helpBtnElem, scoresBtnElem, settingsBtnElem].forEach((el) => {
    if (el) el.classList.add("control-btn");
  });

  // Access Camera overlay: add an Upload Video button next to Grant Permission
  const uploadBtnAccess = document.createElement("div");
  uploadBtnAccess.className =
    "w-full border-2 border-yellow-500 text-yellow-600 py-2 text-xl font-bold rounded-lg hover:bg-yellow-50 text-center cursor-pointer";
  uploadBtnAccess.textContent = "Upload Video";
  if (accessCamBtnElem && accessCamBtnElem.parentElement) {
    accessCamBtnElem.parentElement.appendChild(uploadBtnAccess);
  }

  // Hidden file input dedicated to user-facing Upload buttons
  const uploadVideoBtnExternal = document.createElement("input");
  uploadVideoBtnExternal.type = "file";
  uploadVideoBtnExternal.accept = "video/*";
  uploadVideoBtnExternal.style.display = "none";
  document.body.appendChild(uploadVideoBtnExternal);

  // Trigger the hidden file input on click
  uploadBtnTop.addEventListener("click", () => uploadVideoBtnExternal.click());
  uploadBtnAccess.addEventListener("click", () => uploadVideoBtnExternal.click());

  // Unified handler to process a selected video file for AI assessment
  const handleVideoFile = (file) => {
    ...
    newWebcamElem.setAttribute("src", URL.createObjectURL(file));
    newWebcamElem.load();
    newWebcamElem.play();
  ...
  // Wire external hidden input to the handler
  uploadVideoBtnExternal.addEventListener("change", (event) => {
    if (event.target.files && event.target.files[0]) {
      handleVideoFile(event.target.files[0]);
      // If access camera overlay is open, hide it after selecting a video
      if (accessCamElem) accessCamElem.style.display = "none";
    }
  });
